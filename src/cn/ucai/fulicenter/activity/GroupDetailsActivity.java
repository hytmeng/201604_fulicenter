...
public class GroupDetailsActivity extends BaseActivity implements OnClickListener {
	...
	ArrayList<UserBean> mMembersList;
	GroupBean mGroup;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mMembersList = FuLiCenterApplication.getInstance().getGroupMembers().get(groupId);
		...
		adapter = new GridAdapter(this, R.layout.grid, mMembersList);
			...
			case REQUEST_CODE_EDIT_GROUPNAME: //修改群名称
				...
				if(!TextUtils.isEmpty(returnData)){
					...
					new Thread(new Runnable() {
						public void run() {
							try {
								String path = new ApiParams().with(I.Group.GROUP_NAME, mGroup.getName()).with(I.Group.NEW_NAME,returnData).getRequestUrl(I.REQUEST_UPDATE_GROUP_NAME);
										...
										new Response.Listener<MessageBean>() {
											...
											public void onResponse(MessageBean messageBean) {
												if (messageBean.isSuccess()) {
													...
													setResult(RESULT_OK);
													Intent intent = new Intent("change_group_name");
		...
		new Thread(new Runnable() {
			public void run() {
				try {
					...
					String path = new ApiParams().with(I.Group.GROUP_NAME, mGroup.getName()).with(I.Group.MEMBERS,userName).getRequestUrl(I.REQUEST_DELETE_GROUP_MEMBER);
					executeRequest(new GsonRequest<MessageBean>(path, MessageBean.class,
							new Response.Listener<MessageBean>() {
								@Override
								public void onResponse(MessageBean messageBean) {
		...
		new Thread(new Runnable() {
			public void run() {
				try {
					...
					String path = new ApiParams().with(I.Group.GROUP_NAME, mGroup.getName()).getRequestUrl(I.REQUEST_DELETE_GROUP);
					executeRequest(new GsonRequest<MessageBean>(path, MessageBean.class,
							new Response.Listener<MessageBean>() {
								@Override
								public void onResponse(MessageBean messageBean) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					final ArrayList<UserBean> newUser = new ArrayList<UserBean>();
					if (newmembers != null) {
					...
					String path = new ApiParams().with(I.Group.GROUP_NAME, groupId)
							.with(I.Group.MEMBERS, builder.toString())
...
//				button.setCompoundDrawablesWithIntrinsicBounds(0, R.drawable.smiley_minus_btn, 0, 0);
				...
				if (!group.getOwner().equals(FuLiCenterApplication.getInstance().getUserName())) {
...
//				button.setCompoundDrawablesWithIntrinsicBounds(0, R.drawable.smiley_add_btn, 0, 0);
				// 如果不是创建者或者没有相应权限
				if (!group.isAllowInvites() && !group.getOwner().equals(FuLiCenterApplication.getInstance().getUserName())&&mGroup.isExame() == true) {
					// if current user is not group admin, hide add/remove btn
					convertView.setVisibility(View.INVISIBLE);
				} else {
					// 正处于删除模式下,隐藏添加按钮
...
//				final String username = getItem(position);
				final UserBean item = getItem(position);
...
//				holder.textView.setText(username);
				UserUtils.setUserBeanNick(username, holder.textView);
//				UserUtils.setUserAvatar(getContext(), username, holder.imageView);
				UserUtils.setUserBeanAvatar(username, holder.imageView);
				...
				button.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						if (isInDeleteMode) {
							...
							EMLog.d("group", "remove user from group:" + item.getUserName());
							try {
								String path = new ApiParams().with(I.Group.GROUP_NAME, mGroup.getName()).with(I.Group.MEMBERS,username).getRequestUrl(I.REQUEST_DELETE_GROUP_MEMBER);
								executeRequest(new GsonRequest<MessageBean>(path, MessageBean.class,
										new Response.Listener<MessageBean>() {
											...
											public void onResponse(MessageBean messageBean) {
												...
												if (messageBean.isSuccess()) {
													deleteMembersFromGroup(item.getUserName());
													mMembersList.remove(item);
													notifyDataSetChanged();
												}
											}
										}, errorListener()));
							...
							// user.getUsername()));
							startActivity(new Intent(GroupDetailsActivity.this,UserProfileActivity.class).putExtra("username",username)
