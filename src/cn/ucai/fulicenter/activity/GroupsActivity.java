...
//	protected List<EMGroup> grouplist;
	...
	static final int REQUEST_PUBLIC_GROUP=1;
	static final int REQUEST_ENTER_CHATACTIVITY=2;

	GroupChangeReceivver mGroupChangeReceiver;
	GroupMemberChangeReceiver mGroupMemberChangeReceiver;
...
//		grouplist = EMGroupManager.getInstance().getAllGroups();
		...
		registerGroupMemberChangeReceiver();
		...
		groupListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				if (position == 1) {
					...
					// 添加公开群
					startActivityForResult(new Intent(GroupsActivity.this, PublicGroupsActivity.class), REQUEST_PUBLIC_GROUP);
				} else {
					// 进入群聊
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		switch (requestCode) {
			case REQUEST_NEW_GROUP:
				...
				groupAdapter.addItem(group);//将新建群组显示在列表中
				break;
		}
	}
...
//		grouplist = EMGroupManager.getInstance().getAllGroups();
	...
	protected void onDestroy() {
		...
		if (mGroupChangeReceiver != null) {
			unregisterReceiver(mGroupChangeReceiver);
		}
		if (mGroupMemberChangeReceiver != null) {
			unregisterReceiver(mGroupMemberChangeReceiver);
		}
		super.onDestroy();
		instance = null;
	}
...
//			grouplist = EMGroupManager.getInstance().getAllGroups();
	 ...
	 *
	 */
	class GroupChangeReceivver extends BroadcastReceiver {

		@Override
		public void onReceive(Context context, Intent intent) {
			if (groupAdapter.getCount() == 3 && intent.getAction().equals("update_GroupList")) {
				ArrayList<GroupBean> groupList = FuLiCenterApplication.getInstance().getGrouplist();
				if (groupList.containsAll(mGroupList)) {
					groupAdapter.initList(mGroupList);
				}
			} else if (intent.getAction().equals("change_group_name")) {
	...
	class GroupMemberChangeReceiver extends BroadcastReceiver {

		@Override
		public void onReceive(Context context, Intent intent) {
		GroupBean groupBean= (GroupBean) intent.getSerializableExtra("group");
			groupAdapter.remove(groupBean);
		}
	}

	private void registerGroupChangeReceiver() {
		mGroupChangeReceiver = new GroupChangeReceivver();
		IntentFilter filter=new IntentFilter("update_GroupList");
		filter.addAction("change_group_name");
		registerReceiver(mGroupChangeReceiver, filter);
	}

	private void registerGroupMemberChangeReceiver() {
		mGroupMemberChangeReceiver = new GroupMemberChangeReceiver();
		IntentFilter filter=new IntentFilter("exit_group");
		filter.addAction("delete_group");
		registerReceiver(mGroupMemberChangeReceiver, filter);
	}
	/**
	 * 返回
	 * 
