...
//                userList = FuLiCenterApplication.getInstance().getContactList();
    ...
    public static int sumCartCount(ArrayList<CartBean> cartBeen) {
        int count=0;
        for (int i = 0; i < cartBeen.size(); i++) {
            count += cartBeen.get(i).getCount();
        }
        return count;
    }
    /**
    ...
    public static void addCart(Context context, GoodDetailsBean goods) {
        ...
        for (int i=0;i<cartList.size()&&!isExists;i++) {
            if (goodId == cartList.get(i).getGoodsId()) {
